...
# #     st.markdown(f'**{column_name}**')  # Display column name in bold
# #     if len(values) == 0:
# #         selected_value.append = st.number_input('', key=f"{column_name}_input",min_value=0,max_value=100)
# #     elif len(values) >= 1 and len(values) <= 3:
# #         selected_value.append = st.radio("", values, key=f"{column_name}_radio")
# #     else:  # Use dropdown menu for columns with more than 3 unique values
# #         selected_value.append = st.selectbox("", values, key=f"{column_name}_dropdown")

# # if st.button('Submit'):
# #     # Create a NumPy array of the user input
# #     inputs = np.array([list(selected_value) for column_name, selected_value in column_values.items() if selected_value is not None])

# #     # Predict the price of the diamond
# #     prediction = model.predict(inputs)

# #     # Display the prediction
# #     st.write('The predicted price of the diamond is: $' + str(prediction))
